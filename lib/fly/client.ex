defmodule Fly.Client do
  ...
  # MOD-UZO:: fetch data equivalent to flyctl status:
  def fetch_app_status(name, show_completed, config) do
    ...
    query($name: String!, $show_completed: Boolean!){
      appstatus:app(name: $name) {
        ...
        # original form is: allocations(show_completed: $show_completed)
    ...
    """
    |> perform_query(%{name: name, show_completed: show_completed}, config, :fetch_app_status)
    |> handle_response()
    |> case do
      {:ok, %{"appstatus" => app_status}} ->
        Logger.info("appstatus returned: #{inspect(app_status)}")
        {:ok, app_status}
